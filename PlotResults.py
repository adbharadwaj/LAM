...
def plot_ssd_vs_rank(max_rank):
    ...
    # plt.subplot(211)
    plt.plot([i for i in range(1, len(am_ssds)+1)], am_ssds, label='ANTI-MONOTONE')
    plt.plot([i for i in range(1, len(lam_ssds)+1)], lam_ssds, label='LOOSELY ANTI-MONOTONE')
...
def plot_percent_of_nodes_vs_rank(max_rank, num_of_nodes):
    ...
    # plt.subplot(212)
    plt.plot([i for i in range(1, len(am_percentage_of_nodes_covered)+1)], am_percentage_of_nodes_covered, label='ANTI-MONOTONE')
    plt.plot([i for i in range(1, len(lam_percentage_of_nodes_covered)+1)], lam_percentage_of_nodes_covered, label='LOOSELY ANTI-MONOTONE')
    plt.legend(loc='lower right')
    plt.xlabel('UC Rank')
    # plt.xscale('log')
    plt.ylabel('% Nodes')
